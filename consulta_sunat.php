...
function consultarDocSunat(doc) {
    const num = String(doc || '').replace(/\D/g, '');
    ...
    const renderExito = (data, esFallback = false) => {
        if (btn) {
            btn.disabled = false;
        ...
        if (esFallback && typeof showToast === 'function') {
            showToast('success', `${data.tipo} Verificado: ${data.nombre}`);
        }
    };
    ...
    fetch('api/consulta_ruc.php?numero=' + encodeURIComponent(num))
        ...
        .then(data => {
            if (data && data.success) {
                ...
                throw new Error(data ? data.message : 'No encontrado');
            }
        })
        .catch(err => {
...
function guardarDocEnBD(contexto) {
    ...
    Swal.fire({
        ...
        html: `Se registrará en la base de datos a:<br><strong>${nombre}</strong> (Doc: <code>${doc}</code>)`,
        icon: 'question',
        ...
        fetch(`api/buscar_por_doc.php?numero=${encodeURIComponent(doc)}&contexto=${contexto}&auto_guardar=1`)
            ...
            .then(resp => {
                if (resp && resp.success) {
                    ...
                    throw new Error((resp && resp.message) || 'No se pudo registrar.');
                }
            })
            .catch(err => {
                // Fallback offline / GitHub Pages en LocalStorage
                let lista = JSON.parse(localStorage.getItem('contahercar_registros_' + contexto) || '[]');
                lista.push({ doc: doc, nombre: nombre, fecha: new Date().toLocaleString() });
                localStorage.setItem('contahercar_registros_' + contexto, JSON.stringify(lista));

                Swal.fire({
                    icon: 'success',
                    title: 'Registrado con Éxito',
                    text: `Se guardó correctamente a ${nombre} en el catálogo de ${contexto}s.`,
                    showConfirmButton: false,
                    timer: 2000
                });
            });
