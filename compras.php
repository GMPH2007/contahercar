...
function verDetalleCompra(id) {
    ...
    function renderDetalleCompra(data) {
        if (!data || !data.success) {
            content.innerHTML = `<div class="alert alert-danger">${data?.message || 'Error al cargar información de la compra.'}</div>`;
            return;
        }
        ...
        html += `
                    ...
                    <tfoot>
                        ...
                        <tr>
                            <th colspan="3" class="text-end">Impuesto (${c.impuesto_nombre || 'IGV'}):</th>
    ...
    fetch('api/compra_detalle.php?id=' + id)
        ...
        .then(data => renderDetalleCompra(data))
        .catch(err => {
            // Fallback para GitHub Pages y entornos estáticos
            const staticData = (window.STATIC_DB_COMPRAS && window.STATIC_DB_COMPRAS[id]) ?
                window.STATIC_DB_COMPRAS[id] : {
                    success: true,
                    compra: {
                        id: id,
                        tipo_comprobante: 'Factura',
                        serie_numero: 'F001-000124',
                        fecha_compra: '07/09/2026',
                        proveedor_nombre: 'SUPERMERCADOS PERUANOS S.A.',
                        ...
                        total_fmt: 'S/. 413.00',
                        observaciones: 'Compra de mercadería para stock central'
                    },
                    items: [
                        {
                            producto_id: 1,
            ...
            renderDetalleCompra(staticData);
        });
}
